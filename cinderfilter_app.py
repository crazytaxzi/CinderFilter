...
class CinderFilterAppWindow(CinderWindow):
    ...
    def _migrate_settings(raw: dict) -> dict:
        ...
        values["version"] = CinderFilterAppWindow.SETTINGS_VERSION
        return values
    ...
    def __init__(self) -> None:
        ...
        QTimer.singleShot(350, self._log_layout_geometry)
    ...
    def _log_layout_geometry(self) -> None:
        values = self.layout_measurements()
        self._append_log(
            "Layout geometry: " + ", ".join(f"{key}={value}" for key, value in values.items())
        )
    ...
    def _fill_combo(combo: DarkCombo, choices: list[DeviceChoice], preferred: str) -> None:
        ...
        for index, choice in enumerate(choices):
            clean = CinderFilterAppWindow._clean_device_name(choice)
