...
if (-not $python312) {
    ...
    $python312 = Find-Python312
    if (-not $python312) { throw "Python 3.12 installed, but its executable could not be located." }
}
...
if ($installedHash -ne $requirementsHash) {
    Write-Host "Installing CinderFilter AI dependencies..." -ForegroundColor Cyan
...
Write-Host "Launching CinderFilter Voice Lock v2..." -ForegroundColor Green
& $venvPython "$PSScriptRoot\cinderfilter_voice_lock_v2.py"
